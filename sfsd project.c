...
void defragmentation(FILE *ms, TableAlloc *tableAlloc, int maxBloc) {
    ...
    for (int readPos = 1; readPos < maxBloc; readPos++) {
        ...
        if (buffer.nbenrigstrement > 0) {
            ...
            for (int i = 0; i < buffer.nbenrigstrement; i++) {
                if (!buffer.t[i].supprimé) { // Si l'enregistrement n'est pas supprimé
                    fseek(ms, writePos * sizeof(Bloc), SEEK_SET);
...
void MajTablealloc(FILE *ms, struct TableAlloc *Tablealloc) {
    ...
    while (fread(&buffer, sizeof(Bloc), 1, ms)) {
        ...
        if (s == FB) { // Si tous les enregistrements sont supprimés
...
typedef struct MetaDonnees MetaDonnees;
...
MetaDonnees fichiersMeta[MAX_BLOCKS];
...
void creerFichierChaine(MetaDonnees *meta, int n) {
    ...
    fichiersMeta[fichierMetaCount++] = *meta;
...
void SuppressionLogique(FILE *ms, MetaDonnees *meta, int id) {
    Block buffer;
    int found = 0;
    Block *current = meta->AdressePremierBloc;
...
void suppressionPhysique(FILE *ms, MetaDonnees *meta, int id) {
    Block buffer;
    int found = 0;
    Block *current = meta->AdressePremierBloc;
...
void supprimerFichier(const char *filename, const char *fileToDelete, MetaDonnees *meta) {
...
int main(){
    ...
    if (kbhit()) { // Vérifie si une touche est pressée
            input = getchar(); // Récupère la touche
            if (input == 'q') { // Si l'utilisateur appuie sur 'q'
